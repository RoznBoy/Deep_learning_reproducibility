...
# outputs 리스트에서 실제 레이블(y_true)과 예측 레이블(y_pred)을 추출하여 리스트로 반환합니다.
...
# 예측 결과를 바탕으로 균형 정확도(balanced accuracy) 같은 평가 지표를 계산하여 반환합니다
...
# 주어진 텐서들을 detach하여 계산 그래프에서 분리합니다. 텐서가 리스트나 딕셔너리 형태일 수 있습니다
...
# 입력 배치의 크기를 (batch_size, no_samples, D)에서 (batch_size * no_samples, D)로 변경합니다. 이는 데이터 로더가 하나의 원본 샘플에서 여러 샘플을 생성하는 경우에 사용됩니다.
...
# 주어진 args와 data_module에 따라 모델을 생성합니다. 다양한 모델(mlp, dietnetworks, wpfs, cae, fsnet)을 지원하며, 각각의 모델에 따라 필요한 레이어와 구조를 다르게 설정합니다.
def create_model(args, data_module=None):
	...
	if args.model == 'mlp':
 	...
 	#### WPFS에서 sparsity network를 뺀 모델은 dietmetworks
	#### FirstLinearLater의 parameter 중 is_diet_layer=False 일 경우 Weight Predictor Network 수행하지 않음
	if args.model == 'dietnetworks':
		...
		return GeneralNeuralNetwork(args, first_layer, None)
	elif args.model == 'remove_SPN_wpfs':
		assert args.feature_extractor_dims[0] == args.wpn_layers[-1], "The output size of WPN must be the same as the first layer of the feature extractor."
		assert data_module != None, "You must specify a data_module to compute the feature embeddings"
		
  
		first_layer = FirstLinearLayer(args, is_diet_layer=True, sparsity=None,
						wpn_embedding_matrix=wpn_embedding_matrix, spn_embedding_matrix=spn_embedding_matrix)

		return GeneralNeuralNetwork(args, first_layer, None)
	elif args.model == 'remove_WPN_wpfs':
		assert args.feature_extractor_dims[0] == args.wpn_layers[-1], "The output size of WPN must be the same as the first layer of the feature extractor."
		assert data_module != None, "You must specify a data_module to compute the feature embeddings"
		
  
		first_layer = FirstLinearLayer(args, is_diet_layer=None, sparsity=True,
						wpn_embedding_matrix=wpn_embedding_matrix, spn_embedding_matrix=spn_embedding_matrix)
...
# 인코더의 두 부분을 구성하는 선형 레이어들을 생성합니다. 각각의 부분은 활성화 함수, 배치 정규화 및 드롭아웃을 포함합니다
...
# 첫 번째 선형 레이어를 구현하며, 다이어트 레이어(Diet Layer)와 희소성 네트워크(Sparsity Network)를 포함할 수 있습니다.
# 다이어트 레이어가 활성화된 경우 Weight Predictor Network를 사용하여 가중치를 예측합니다.
# Sparsity Network가 활성화된 경우 희소성 가중치를 적용하여 입력 데이터를 처리합니다.
...
# Concrete Autoencoders for Differentiable Feature Selection and Reconstruction" 논문에서 제안된 콘크리트 레이어를 구현합니다.
# 콘크리트 분포에서 샘플링하여 입력 데이터의 특정 피처를 선택합니다.
# cae, fsnet 모델에서만 사용
...
# 다이어트 네트워크와 함께 사용되는 디코더를 구현합니다. 가중치 예측 네트워크를 사용하여 입력 데이터의 재구성을 수행합니다
...
# 다양한 모델을 통합하여 일반적인 신경망 구조를 구현합니다.
# 첫 번째 레이어, 인코더, 디코더, 분류 레이어 등을 포함합니다.
# 훈련, 검증, 테스트 단계에서의 데이터 처리와 손실 계산을 담당합니다.
